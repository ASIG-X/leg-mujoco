...
if not hasattr(jax, "device_put_replicated"):
  ...
  def _device_put_replicated(tree, devices):
    """Compatibility shim for Brax versions using older JAX APIs."""
    devices = list(devices)
    replicated = jax.tree_util.tree_map(
        lambda x: jp.stack([x] * len(devices)), tree
    )
    if hasattr(jax.sharding, "PmapSharding"):
      return jax.device_put(
          replicated, jax.sharding.PmapSharding.default(devices)
      )
...
_WANDB_PROJECT = flags.DEFINE_string(
    "wandb_project", None, "Weights & Biases project name"
)
_WANDB_ENTITY = flags.DEFINE_string(
    "wandb_entity", None, "Optional Weights & Biases entity/team"
)
_WANDB_NAME = flags.DEFINE_string(
    "wandb_name", None, "Optional suffix for the generated W&B run name"
)
_WANDB_MODE = flags.DEFINE_string(
    "wandb_mode", "online", "Weights & Biases execution mode"
)
_WANDB_LOG_INTERVAL = flags.DEFINE_integer(
    "wandb_log_interval", 1, "Log to Weights & Biases every N progress calls"
)
...
def _sanitize_run_name(name: str) -> str:
  """Keep user-provided run names safe for directory names."""
  if not name:
    return ""
  safe = "".join(
...
def main(argv):
  ...
  safe_wandb_name = _sanitize_run_name(_WANDB_NAME.value)
  run_suffix = f"_{safe_wandb_name}" if safe_wandb_name else ""
  exp_name = f"jave_{timestamp}{run_suffix}"
  if _SUFFIX.value is not None:
    exp_name += f"-{_sanitize_run_name(_SUFFIX.value)}"
  ...
  if _USE_WANDB.value and not _PLAY_ONLY.value:
    if wandb is None:
      raise ImportError(
          ...
          "or disable logging with `--use_wandb=false`."
      )
    wandb_config = {
        "algorithm": "ppo",
        ...
        "wandb_log_interval": _WANDB_LOG_INTERVAL.value,
  ...
  def progress(num_steps, metrics):
    ...
    if wandb_run is not None:
      progress_calls[0] += 1
      if progress_calls[0] % max(_WANDB_LOG_INTERVAL.value, 1) == 0:
        scalar_metrics = {
            name: float(value) for name, value in metrics.items()
        }
        wandb_metrics = {
            "train/step": int(num_steps),
            "train/reward_mean": scalar_metrics.get(
                "eval/episode_reward", scalar_metrics.get("episode_reward", 0.0)
            ),
            "train/episode_length": scalar_metrics.get(
                "eval/episode_length", 0.0
            ),
        }
  ...
  if wandb_run is not None:
    wandb_run.summary["final_step"] = int(ppo_params.num_timesteps)
    wandb_run.summary["elapsed_seconds"] = times[-1] - times[0]
    wandb_run.summary["save_dir"] = str(logdir)
  ...
  media.write_video(f"logs/videos/{safe_wandb_name}_rollout.mp4", frames, fps=fps)
  print("Rollout video saved as 'logs/videos/rollout.mp4'.")
