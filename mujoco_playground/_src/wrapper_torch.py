...
class RSLRLBraxWrapper(VecEnv):
  ...
  def step(self, action):
    ...
    info_ret = {
        ...
        "log": {
            "reward/total": reward.float().mean().item(),
        },
    }
  ...
  def reset(self):
    ...
    return self._obs_to_tensordict()
  ...
  def _obs_to_tensordict(self):
    from tensordict import TensorDict  # pylint: disable=import-outside-toplevel

    if self.asymmetric_obs:
      obs = {
          "state": _jax_to_torch(self.env_state.obs["state"]),
          "privileged_state": _jax_to_torch(
              self.env_state.obs["privileged_state"]
          ),
      }
    else:
      obs = {"state": _jax_to_torch(self.env_state.obs)}
